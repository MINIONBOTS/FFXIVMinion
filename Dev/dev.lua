...
function dev.DisplaySelectableCell(cellInfo, cellData, checkMouseActions)
	...
	if cellInfo.info:len() == 0 then
		cellInfo.info = tostring(cellData)
	else
									...
									if (table.valid(ilist)) then
										for slot, item in pairs(ilist) do
											if ( GUI:TreeNode(tostring(slot).." - "..item.name)) then
												...
												GUI:BulletText("In Gearset") GUI:SameLine(225) GUI:InputText("##devbag19b"..tostring(slot),tostring(item.isingearset))
												...
												GUI:BulletText("SpiritbondValue") GUI:SameLine(225) GUI:InputText("##devbag18b"..tostring(slot),tostring(item.spiritbondvalue))
