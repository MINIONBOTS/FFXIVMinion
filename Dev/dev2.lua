			...
			if ( GUI:TreeNode("Scanner") ) then
				if( gamestate == FFXIV.GAMESTATE.INGAME ) then 
					...
					if (table.valid(el)) then
						GUI:Columns(17, "##dev-scanner-details",true)
						...
						GUI:Text("X"); GUI:NextColumn()
						GUI:Text("Y"); GUI:NextColumn()
						GUI:Text("Z"); GUI:NextColumn()
						GUI:Text("MX"); GUI:NextColumn()
						GUI:Text("MY"); GUI:NextColumn()
						GUI:Text("Follow"); GUI:NextColumn()
						GUI:Text("Angle"); GUI:NextColumn()
						GUI:Text("Pitch"); GUI:NextColumn()
						...
						for i, entity in pairs(el) do
							...
							local x,y,z,angle,pitch,mx,my,btn = "","","","","","","",""
							if (table.valid(entity.pos)) then
								x = entity.pos.x
								y = entity.pos.y
								z = entity.pos.z

								local mPos = RenderManager:WorldToScreen(entity.pos)
								if (table.valid(mPos)) then
									mx = mPos.x
									my = mPos.y	
									btn = "Follow##"..tostring(entity.id)
								end
								
								local ppos = shallowcopy(Player.pos)
								local goalpos = {x=602.87,y=176.98,z=-613.55,h=0}
								local mainAngle = DegreesToHeading(AngleFromPos(ppos, goalpos))
								local mainHeading = ConvertHeading(mainAngle)

								local entityAngle = DegreesToHeading(AngleFromPos(ppos, entity.pos))
								local entityHeading = ConvertHeading(entityAngle)

								angle = "mainAngle:"..tostring(mainAngle)..",entityAngle:"..tostring(entityAngle)	

								local minVector = math.normalize(math.vectorize(ppos,entity.pos))
								pitch = math.asin(-1 * minVector.y)
							end
							...
							GUI:Text(my); GUI:NextColumn()
							if (btn ~= "") then
								if (GUI:Button(btn)) then
									Player:SetSnipeCam(mx,my,0)
								end
							else
								GUI:Text("")
							end
			...
			if ( GUI:TreeNode("Player specific Data")) then
				if( gamestate == FFXIV.GAMESTATE.INGAME ) then
					...
					if ( GUI:TreeNode("Snipe Cam")) then
						...
						GUI:BulletText("current x, y, zoom") GUI:SameLine(200)  GUI:InputFloat3( "##dev9", snp.x, snp.y, snp.zoom, 2, GUI.InputTextFlags_ReadOnly) 
						local mousepos = GetMouseInWorldPos()
						GUI:BulletText("mouse world pos") GUI:SameLine(200)  GUI:InputFloat3( "##dev9a", mousepos.x, mousepos.y, mousepos.z, 2, GUI.InputTextFlags_ReadOnly) 
						if (gDevSnipeX == nil) then gDevSnipeX = 0 end
						if (gDevSnipeY == nil) then gDevSnipeY = 0 end
						if (gDevSnipeZoom == nil) then gDevSnipeZoom = 0 end
						GUI:BulletText("set x, y, zoom") GUI:SameLine(200)  gDevSnipeX,gDevSnipeY,gDevSnipeZoom = GUI:InputFloat3( "##dev9b", gDevSnipeX, gDevSnipeY, gDevSnipeZoom, 2) 
						if (GUI:Button("set cam",100,15)) then
							Player:SetSnipeCam(gDevSnipeX,gDevSnipeY,gDevSnipeZoom)
						end
