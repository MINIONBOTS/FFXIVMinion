...
function GetNearestGrindAttackable()
	...
	local el = EntityList("lowesthealth,alive,attackable,onmesh,maxdistance="..tostring(ml_global_information.AttackRange)..",minLevel="..tostring(level-3)..",maxlevel="..tostring(level+3))
	if ( el ) then
		local i,e = next(el)
