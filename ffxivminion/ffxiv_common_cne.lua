...
function c_walktopos:evaluate()
    ...
    if (table.valid(ml_task_hub:CurrentTask().pos) or table.valid(ml_task_hub:CurrentTask().gatePos)) then	
		...
		if (ml_task_hub:CurrentTask().gatePos) then
			...
			d("[c_walktopos]: Position adjusted to gate position.", "gLogCNE", 2)
		else
			gotoPos = ml_task_hub:CurrentTask().pos
			d("[c_walktopos]: Position left as original position.", "gLogCNE", 2)
		end
		...
		if (table.valid(gotoPos)) then
			...
			if (target) then
				if (ml_task_hub:CurrentTask().contentid == target.contentid) then
					...
					if (distFromGoal <= 5) then
						if (target.distance2d < 2.5 and target.los) then
							if (Player:IsMoving()) then
			...
			d("range required:"..tostring(range))
			if (range > 0) then
				local distance = 0.0
...
function c_acceptquest:evaluate()
	if (gBotMode == GetString("assistMode") and not FFXIV_Assist_QuestHelpers) then
		return false
	end
