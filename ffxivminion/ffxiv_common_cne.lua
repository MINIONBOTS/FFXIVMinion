...
c_roleautoset = inheritsFrom( ml_cause )
e_roleautoset = inheritsFrom( ml_effect )
e_roleautoset.activity = ""
e_roleautoset.lastRole = {}
function c_roleautoset:evaluate()

	if (Player.level == 70) or Player.incombat or not IsNormalMap(Player.localmapid) then
		return false
	end
	
	if (table.valid(e_roleautoset.lastRole)) then
		if (e_roleautoset.lastRole[Player.job] and e_roleautoset.lastRole[Player.job] >= Player.level) then
			--d("Returned false same level as last set")
			return false
		end	
	end
	...
	if gRoleSkillAutoset then
		if IsTank(Player.job) then
			...
			local lowBlow = ActionList:Get(1,7540)
			local hasLowBlow = lowBlow.usable
			local provoke = ActionList:Get(1,7533)
			local hasProvoke = provoke.usable
			local convalescence = ActionList:Get(1,7532)
			local hasConvalescence = convalescence.usable
			local anticipation = ActionList:Get(1,7536)
			local hasAnticipation = anticipation.usable
			local reprisal = ActionList:Get(1,7535)
			local hasReprisal = reprisal.usable
			local awareness = ActionList:Get(1,7534)
			local hasAwareness = awareness.usable
			local interject = ActionList:Get(1,7538)
			local hasInterject = interject.usable
			local ultimatum = ActionList:Get(1,7539)
			local hasUltimatum = ultimatum.usable
			...
			if Player.level >= rampart.level and (gRoleSkillRampart and not hasRampart) or (not gRoleSkillRampart and hasRampart) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "userampart"
					return true
				end
			end
			if Player.level >= lowBlow.level and (gRoleSkillLowBlow and not hasLowBlow) or (not gRoleSkillLowBlow and hasLowBlow) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "uselowBlow"
					return true
				end
			end
			if Player.level >= provoke.level and (gRoleSkillProvoke and not hasProvoke) or (not gRoleSkillProvoke and hasProvoke) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useprovoke"
					return true
				end
			end
			if Player.level >= convalescence.level and (gRoleSkillConvalescence and not hasConvalescence) or (not gRoleSkillConvalescence and hasConvalescence) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useconvalescence"
					return true
				end
			end
			if Player.level >= anticipation.level and (gRoleSkillAnticipation and not hasAnticipation) or (not gRoleSkillAnticipation and hasAnticipation) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useanticipation"
					return true
				end
			end
			if Player.level >= reprisal.level and (gRoleSkillReprisal and not hasReprisal) or (not gRoleSkillReprisal and hasReprisal) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usereprisal"
					return true
				end
			end
			if Player.level >= awareness.level and (gRoleSkillAwareness and not hasAwareness) or (not gRoleSkillAwareness and hasAwareness) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useawareness"
					return true
				end
			end
			if Player.level >= interject.level and (gRoleSkillInterject and not hasInterject) or (not gRoleSkillInterject and hasInterject) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useinterject"
					return true
				end
			end
			if Player.level >= ultimatum.level and (gRoleSkillUltimatum and not hasUltimatum) or (not gRoleSkillUltimatum and hasUltimatum) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useultimatum"
					return true
				end
			end
			if Player.level >= shirk.level and (gRoleSkillShirk and not hasShirk) or (not gRoleSkillShirk and hasShirk) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useshirk"
					return true
				end
			end
		end
		
		if IsHealer(Player.job) then
			local clericstance = ActionList:Get(1,7567)
			local hasClericstance = clericstance.usable
			local breakSkill = ActionList:Get(1,7558)
			local hasBreakSkill = breakSkill.usable
			local protect = ActionList:Get(1,7572)
			local hasProtect = protect.usable
			local esuna = ActionList:Get(1,7568)
			local hasEsuna = esuna.usable
			local lucid = ActionList:Get(1,7562)
			local hasLucid = lucid.usable
			local swiftcast = ActionList:Get(1,7561)
			local hasSwiftcast = swiftcast.usable
			local eye = ActionList:Get(1,7569)
			local hasEye = eye.usable
			local largesse = ActionList:Get(1,7570)
			local hasLargesse = largesse.usable
			local surecast = ActionList:Get(1,7559)
			local hasSurecast = surecast.usable
			...
			if Player.level >= clericstance.level and (gRoleSkillClericstance and not hasClericstance) or (not gRoleSkillClericstance and hasClericstance) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useclericstance"
					return true
				end
			end
			if Player.level >= breakSkill.level and (gRoleSkillBreakSkill and not hasBreakSkill) or (not gRoleSkillBreakSkill and hasBreakSkill) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usebreakSkill"
					return true
				end
			end
			if Player.level >= protect.level and (gRoleSkillProtect and not hasProtect) or (not gRoleSkillProtect and hasProtect) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useprotect"
					return true
				end
			end
			if Player.level >= esuna.level and (gRoleSkillEsuna and not hasEsuna) or (not gRoleSkillEsuna and hasEsuna) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useesuna"
					return true
				end
			end
			if Player.level >= lucid.level and (gRoleSkillLucid and not hasLucid) or (not gRoleSkillLucid and hasLucid) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "uselucid"
					return true
				end
			end
			if Player.level >= swiftcast.level and (gRoleSkillSwiftcast and not hasSwiftcast) or (not gRoleSkillSwiftcast and hasSwiftcast) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useswiftcast"
					return true
				end
			end
			...
			if Player.level >= largesse.level and (gRoleSkillLargesse and not hasLargesse) or (not gRoleSkillLargesse and hasLargesse) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "uselargesse"
					return true
				end
			end
			if Player.level >= surecast.level and (gRoleSkillSurecast and not hasSurecast) or (not gRoleSkillSurecast and hasSurecast) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usesurecast"
					return true
				end
			end
			if Player.level >= rescue.level and (gRoleSkillRescue and not hasRescue) or (not gRoleSkillRescue and hasRescue) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "userescue"
					return true
				end
			end
		end
		if IsCasterDPS(Player.job) then
			...
			local hasAddle = addle.usable
			local breakSkill = ActionList:Get(1,7558)
			local hasBreakSkill = breakSkill.usable
			local drain = ActionList:Get(1,7564)
			local hasDrain = drain.usable
			...
			local swiftcast = ActionList:Get(1,7561)
			local hasSwiftcast = swiftcast.usable
			local manaShift = ActionList:Get(1,7565)
			local hasManaShift = manaShift.usable
			local apoc = ActionList:Get(1,7563)
			local hasApoc = apoc.usable
			local surecast = ActionList:Get(1,7559)
			local hasSurecast = surecast.usable
			...
			if Player.level >= addle.level and (gRoleSkillAddle and not hasAddle) or (not gRoleSkillAddle and hasAddle) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useaddle"
					return true
				end
			end
			if Player.level >= breakSkill.level and (gRoleSkillBreakSkill and not hasBreakSkill) or (not gRoleSkillBreakSkill and hasBreakSkill) then
			...
			if Player.level >= drain.level and (gRoleSkillDrain and not hasDrain) or (not gRoleSkillDrain and hasDrain) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usedrain"
					return true
				end
			end
			if Player.level >= diversion.level and (gRoleSkillDiversion and not hasDiversion) or (not gRoleSkillDiversion and hasDiversion) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usediversion"
					return true
				end
			end
			if Player.level >= lucid.level and (gRoleSkillLucid and not hasLucid) or (not gRoleSkillLucid and hasLucid) then
			...
			if Player.level >= manaShift.level and (gRoleSkillMana and not hasManaShift) or (not gRoleSkillMana and hasManaShift) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usemanaShift"
					return true
				end
			end
			if Player.level >= apoc.level and (gRoleSkillApoc and not hasApoc) or (not gRoleSkillApoc and hasApoc) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useapoc"
					return true
				end
			end
			if Player.level >= surecast.level and (gRoleSkillSurecast and not hasSurecast) or (not gRoleSkillSurecast and hasSurecast) then
			...
			if Player.level >= erase.level and (gRoleSkillErase and not hasErase) or (not gRoleSkillErase and hasErase) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useerase"
					return true
				end
			end
		end
		if In(Player.job,5,23,31) then
			local secondWind = ActionList:Get(1,7541)
			local hasSecondWind = secondWind.usable
			local invigorate = ActionList:Get(1,7544)
			local hasInvigorate = invigorate.usable
			local headGraze = ActionList:Get(1,7551)
			local hasHeadGraze = headGraze.usable
			local armGraze = ActionList:Get(1,7552)
			local hasArmGraze = armGraze.usable
			local footGraze = ActionList:Get(1,7553)
			local hasFootGraze = footGraze.usable
			local legGraze = ActionList:Get(1,7554)
			local hasLegGraze = legGraze.usable
			local tactician = ActionList:Get(1,7555)
			local hasTactician = tactician.usable
			local refresh = ActionList:Get(1,7556)
			local hasRefresh = refresh.usable
			local palisade = ActionList:Get(1,7550)
			local hasPalisade = palisade.usable
			...
			if Player.level >= secondWind.level and (gRoleSkillRangedSecondWind and not hasSecondWind) or (not gRoleSkillRangedSecondWind and hasSecondWind) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usesecondwind"
					return true
				end
			end
			if Player.level >= footGraze.level and (gRoleSkillFootGraze and not hasFootGraze) or (not gRoleSkillFootGraze and hasFootGraze) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usefootgraze"
					return true
				end
			end
			if Player.level >= legGraze.level and (gRoleSkillLegGraze and not hasLegGraze) or (not gRoleSkillLegGraze and hasLegGraze) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useleggraze"
					return true
				end
			end
			if Player.level >= peloton.level and (gRoleSkillPeloton and not hasPeloton) or (not gRoleSkillPeloton and hasPeloton) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usepeloton"
					return true
				end
			end
			if Player.level >= invigorate.level and (gRoleSkillRangedInvigorate and not hasInvigorate) or (not gRoleSkillRangedInvigorate and hasInvigorate) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useinvigorate"
					return true
				end
			end
			if Player.level >= tactician.level and (gRoleSkillTactician and not hasTactician) or (not gRoleSkillTactician and hasTactician) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usetactician"
					return true
				end
			end
			if Player.level >= refresh.level and (gRoleSkillRefresh and not hasRefresh) or (not gRoleSkillRefresh and hasRefresh) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "userefresh"
					return true
				end
			end
			if Player.level >= headGraze.level and (gRoleSkillHeadGraze and not hasHeadGraze) or (not gRoleSkillHeadGraze and hasHeadGraze) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "useheadgraze"
					return true
				end
			end
			if Player.level >= armGraze.level and (gRoleSkillArmGraze and not hasArmGraze) or (not gRoleSkillArmGraze and hasArmGraze) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usearmgraze"
					return true
				end
			end
			if Player.level >= palisade.level and (gRoleSkillPalisade and not hasPalisade) or (not gRoleSkillPalisade and hasPalisade) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usepalisade"
					return true
				end
			end
		end
		if In(Player.job,2,4,20,22,29,30,34) then
			local secondWind = ActionList:Get(1,7541)
			local hasSecondWind = secondWind.usable
			local invigorate = ActionList:Get(1,7544)
			local hasInvigorate = invigorate.usable
			local armsLength = ActionList:Get(1,7548)
			local hasArmsLength = armsLength.usable
			local bloodbath = ActionList:Get(1,7542)
			local hasBloodbath = bloodbath.usable
			local goad = ActionList:Get(1,7543)
			local hasGoad = goad.usable
			local trueNorth = ActionList:Get(1,7546)
			local hasTrueNorth = trueNorth.usable
			local crutch = ActionList:Get(1,7547)
			local hasCrutch = crutch.usable
			local feint = ActionList:Get(1,7549)
			local hasFeint = feint.usable
			local legSweep = ActionList:Get(1,7863)
			local hasLegSweep = legSweep.usable
			local diversion = ActionList:Get(1,7545)
			local hasDiversion = diversion.usable
			...
			if Player.level >= armsLength.level and (gRoleSkillArmsLength and not hasArmsLength) or (not gRoleSkillArmsLength and hasArmsLength) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usearmslength"
					return true
				end
			end
			if Player.level >= legSweep.level and (gRoleSkillLegSweep and not hasLegSweep) or (not gRoleSkillLegSweep and hasLegSweep) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "uselegsweep"
					return true
				end
			end
			...
			if Player.level >= bloodbath.level and (gRoleSkillBloodbath and not hasBloodbath) or (not gRoleSkillBloodbath and hasBloodbath) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usebloodbath"
					return true
				end
			end
			if Player.level >= goad.level and (gRoleSkillGoad and not hasGoad) or (not gRoleSkillGoad and hasGoad) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usegoad"
					return true
				end
			end
			if Player.level >= feint.level and (gRoleSkillFeint and not hasFeint) or (not gRoleSkillFeint and hasFeint) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usefeint"
					return true
				end
			end
			if Player.level >= crutch.level and (gRoleSkillCrutch and not hasCrutch) or (not gRoleSkillCrutch and hasCrutch) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usecrutch"
					return true
				end
			end
			if Player.level >= trueNorth.level and (gRoleSkillTrueNorth and not hasTrueNorth) or (not gRoleSkillTrueNorth and hasTrueNorth) then
				if (not table.valid(e_roleautoset.lastRole)) or (table.valid(e_roleautoset.lastRole) and 
					...
					e_roleautoset.activity = "usetruenorth"
					return true
				end
			end
		end
	end
	e_roleautoset.lastRole[Player.job] = Player.level
end

function e_roleautoset:execute()
	local activity = e_roleautoset.activity
	d(activity)
	
	if (activity == "userampart") then
		local action = ActionList:Get(1,7531)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "uselowBlow") then
			local action = ActionList:Get(1,7540)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useprovoke") then
			local action = ActionList:Get(1,7533)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useconvalescence") then
			local action = ActionList:Get(1,7532)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useanticipation") then
			local action = ActionList:Get(1,7536)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usereprisal") then
			local action = ActionList:Get(1,7535)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useawareness") then
			local action = ActionList:Get(1,7534)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useinterject") then
			local action = ActionList:Get(1,7538)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useultimatum") then
			local action = ActionList:Get(1,7539)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useshirk") then
			local action = ActionList:Get(1,7537)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	
	if (activity == "useclericstance") then
		local action = ActionList:Get(1,7567)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usebreakSkill") then
			local action = ActionList:Get(1,7558)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useprotect") then
			local action = ActionList:Get(1,7572)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useesuna") then
			local action = ActionList:Get(1,7568)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "uselucid") then
			local action = ActionList:Get(1,7562)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useswiftcast") then
			local action = ActionList:Get(1,7561)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useeye") then
			local action = ActionList:Get(1,7569)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "uselargesse") then
			local action = ActionList:Get(1,7570)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usesurecast") then
			local action = ActionList:Get(1,7559)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "userescue") then
			local action = ActionList:Get(1,7571)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useaddle") then
			local action = ActionList:Get(1,7560)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usedrain") then
			local action = ActionList:Get(1,7564)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usediversion") then
			local action = ActionList:Get(1,7545)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usemanaShift") then
			local action = ActionList:Get(1,7565)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useapoc") then
			local action = ActionList:Get(1,7563)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useerase") then
			local action = ActionList:Get(1,7566)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usesecondwind") then
			local action = ActionList:Get(1,7541)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usefootgraze") then
			local action = ActionList:Get(1,7553)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useleggraze") then
			local action = ActionList:Get(1,7554)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usepeloton") then
			local action = ActionList:Get(1,7557)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useinvigorate") then
			local action = ActionList:Get(1,7544)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usetactician") then
			local action = ActionList:Get(1,7555)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "userefresh") then
			local action = ActionList:Get(1,7556)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "useheadgraze") then
			local action = ActionList:Get(1,7551)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usearmgraze") then
			local action = ActionList:Get(1,7552)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usepalisade") then
			local action = ActionList:Get(1,7550)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usearmslength") then
			local action = ActionList:Get(1,7548)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "uselegsweep") then
			local action = ActionList:Get(1,7863)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usebloodbath") then
			local action = ActionList:Get(1,7542)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usegoad") then
			local action = ActionList:Get(1,7543)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usefeint") then
			local action = ActionList:Get(1,7549)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usecrutch") then
		local action = ActionList:Get(1,7547)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
	if (activity == "usetruenorth") then
		local action = ActionList:Get(1,7546)
		SendTextCommand('/aaction "'..action.name..'"')
		ml_global_information.Await(2500)
	end
end
...
function c_skipcutscene:evaluate()
	if (gSkipCutscene and FFXIV_Common_BotRunning and not IsControlOpen("Snipe") and not IsControlOpen("JournalResult") and TimeSince(c_skipcutscene.lastSkip) > 3000) then
		...
		if (In(totalUI,4725,5701,3451,2628,2626,2893,3506) and not IsControlOpen("NowLoading")) then
			if (IsControlOpen("SelectString") or IsControlOpen("SelectIconString")) then
				local convoList = GetConversationList()
				if (table.valid(convoList)) then
