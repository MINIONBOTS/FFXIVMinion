	...
	if (ValidTable(data)) then
		...
		for i,instruction in pairsByKeys(data) do
			...
			elseif (itype == "Action") then
				local actionid = tonumber(iparams[1]) or 0
				local actiontype = tonumber(iparams[2]) or 0
				local targetid = tonumber(iparams[3]) or 0

				table.insert(ml_navigation.receivedInstructions,
					function ()
						local action = ActionList:Get(actiontype,actionid)
						if (action) then
							if (action.isoncd and ((action.cdmax - action.cd) > 2.5)) then
								return true
							else
...
function ml_navigation.GetInteractVerticalSeparation(ppos, tpos, target)
	if target then
		local planarDist = tonumber(target.distance2d)
		local dist3d = tonumber(target.distance)
		if planarDist and dist3d and dist3d > 0 and planarDist < dist3d then
...
function ml_navigation:GetLandingDispatchNode(surfaceNode)
	if not table.valid(surfaceNode) then return nil end
	local surfaceY = surfaceNode.surfaceY or surfaceNode.y or 0
	return {
		x = surfaceNode.x,
		y = self:GetLandingSurfaceDispatchY(surfaceY),
		z = surfaceNode.z,
		landing = true,
		surfaceY = surfaceY,
	}
...
function ml_navigation:GetLandingRequest(task)
	...
	if not self:IsInteractActionTask(task) then return nil end
...
function ml_navigation:ResolveLandingSite(request, approachPos)
	...
	if cachedLanding then
		if (gNavDebug and self.DebugLog) then
...
function ml_navigation:IsFlightSegmentClear(fromPos, toPos, radius)
	if not (table.valid(fromPos) and table.valid(toPos)) then return false end
	radius = math.max(tonumber(radius) or 3.5, 1.0)
	if (CheckFlightSegment) then
		local clear, hx, hy, hz, flag1, flag2, hitDist = CheckFlightSegment(
			fromPos.x, fromPos.y, fromPos.z, toPos.x, toPos.y, toPos.z, radius)
		if not clear and hx and hy and hz then
			local hitPos = { x = hx, y = hy, z = hz }
			local hitFromStart = math.distance3d(hitPos, fromPos)
			local hitToTarget = math.distance3d(hitPos, toPos)
			local segmentLen = math.distance3d(fromPos, toPos)
			local endTolerance = math.max(1.25, radius + 0.25)
			local hitDistance = tonumber(hitDist)
			if hitFromStart <= endTolerance
				or hitToTarget <= endTolerance
				or (hitDistance and (hitDistance <= endTolerance
					or (segmentLen > 0 and (segmentLen - hitDistance) <= endTolerance))) then
				ffnav.lastFlightSegmentHit = nil
				return true
			end
		end
		ffnav.lastFlightSegmentHit = (not clear and hx and hy and hz) and {
			x = hx, y = hy, z = hz, flag1 = flag1, flag2 = flag2, distance = hitDist,
		} or nil
		return clear == true
	end
	...
	local offsets = {
		{ 0, -1.75, 0 }, { sideX * side, -1.5, sideZ * side },
		{ -sideX * side, -1.5, -sideZ * side },
...
function ml_navigation:IsLandingSegmentClear(fromPos, toPos, radius)
	return self:IsFlightSegmentClear(fromPos, toPos, radius)
end
...
function ml_navigation:BuildLandingRoute(fromPos, landingPos, radius, quality)
	...
	local land = self:GetLandingDispatchNode(landingPos)
	if self:IsLandingSegmentClear(fromPos, hover, radius) then return { hover, land } end
...
function ml_navigation:PlanFlyingLanding(controller, request, ppos)
	...
	self:SuppressUnstuck(2500, "LandingPlan")
	if not route then
...
function ml_navigation:DriveFlyingLanding(controller, request, ppos)
	if controller.phase == 'blocked' then
		self:ResetLandingController()
		return false
	end
	if controller.phase == 'dismount' then
		...
		if self:IsLandingControllerWaterSurfaceReady(controller, ppos) then
			controller.airLandingStartedAt = Now()
			self:SuppressUnstuck(3000, "LandingWaterDismount")
			if (gNavDebug and self.DebugLog) then
				self:DebugLog("landing-water-dismount",
					"Water landing surface detected; retrying dismount at "
					.. self:FormatGroundAcquirePoint(controller.landing), 1000)
			end
...
function ml_navigation:ContinueFlyingLanding(controller, request, ppos)
	...
	if not self:PrepareLandingController(controller, request, ppos) then return false end
...
function ml_navigation:ManageFlyingLanding(request, ppos)
	...
	self:SuppressUnstuck(1500, "LandingManage")
	return self:ContinueFlyingLanding(controller, request, ppos)
end

function ml_navigation:SuppressUnstuck(duration, reason)
	local untilTime = Now() + (tonumber(duration) or 1500)
	if (not ffnav.suppressUnstuckUntil or ffnav.suppressUnstuckUntil < untilTime) then
		ffnav.suppressUnstuckUntil = untilTime
		ffnav.suppressUnstuckReason = reason or "navigation"
	end
...
function ml_navigation:UpdateNavigationHandoff(task)
	...
	if handoff.kind == "airLanding" then
		if self:IsAirLandingResolved() then
			if Player.ismounted and not self:IsPlayerSwimming() then
	...
	elseif handoff.kind == "landingTakeoff" then
		if self:IsCurrentWalkingPathSegment(task, Player and Player.pos) then
...
function ml_navigation:IsPlayerSwimming()
	return (type(IsSwimming) == "function" and IsSwimming()) or false
end

function ml_navigation:IsAirLandingResolved()
	return (not IsFlying()) or self:IsPlayerSwimming() or (Player and Player.ismounted == false)
end

function ml_navigation:ProbeWaterSurfaceAt(pos, verticalTolerance, source)
	if not (table.valid(pos) and type(Raycast2) == "function") then return false end
	verticalTolerance = tonumber(verticalTolerance) or 4
	local WATER_FLAG = 0x8000
	local PASSTHRU_BOUNDARIES = 0x4000
	local function hasWaterFlag(raw1, raw2)
		raw1 = tonumber(raw1) or 0
		raw2 = tonumber(raw2) or 0
		return (bit.band(raw1, WATER_FLAG) ~= 0) or (bit.band(raw2, WATER_FLAG) ~= 0)
	end

	local probes = {
		{ mask = WATER_FLAG, value = WATER_FLAG, name = "water" },
		{ mask = PASSTHRU_BOUNDARIES, value = PASSTHRU_BOUNDARIES, name = "passthru" },
	}
	local lastHit, lastY, lastRaw1, lastRaw2, lastProbe = false, nil, nil, nil, nil
	for _, probe in ipairs(probes) do
		local hit, hx, hy, hz, ax, ay, az, bx, by, bz, cx, cy, cz, raw1, raw2 = Raycast2(
			pos.x, (pos.y or 0) + 8, pos.z,
			pos.x, (pos.y or 0) - 40, pos.z,
			probe.mask, probe.value)
		lastHit, lastY, lastRaw1, lastRaw2, lastProbe = hit, hy, raw1, raw2, probe.name
		if hit and hasWaterFlag(raw1, raw2) then
			if (not hy) or math.abs((hy or 0) - (pos.y or 0)) <= verticalTolerance then
				return true, { x = hx or pos.x, y = hy or pos.y, z = hz or pos.z }
			end
		end
	end

	if gNavDebug and self.DebugLog then
		self:DebugLog("water-surface-probe-fail",
			"Water probe failed source=" .. tostring(source or "unknown")
			.. " pos=" .. self:FormatGroundAcquirePoint(pos)
			.. " hit=" .. tostring(lastHit)
			.. " y=" .. tostring(lastY)
			.. " raw1=" .. tostring(lastRaw1)
			.. " raw2=" .. tostring(lastRaw2)
			.. " probe=" .. tostring(lastProbe), 1000)
	end
	return false
end

function ml_navigation:IsWaterSurfaceDismountReady(task, ppos, goal)
	if not (task and Player and Player.ismounted and IsFlying()) then return false end
	if (type(IsDiving) == "function" and IsDiving()) then return false end
	if not (table.valid(ppos) and table.valid(goal)) then return false end
	if type(CanAirDismountAtPos) ~= "function" or type(Raycast2) ~= "function" then return false end

	local dismountDistance = tonumber(task.dismountDistance) or 5
	local scanDistance = math.max(dismountDistance + 3, 8)
	local verticalTolerance = math.max(tonumber(task.dismountHeightTolerance) or 3.5, 4)
	if math.distance2d(ppos, goal) > scanDistance then return false end
	if math.abs((ppos.y or 0) - (goal.y or 0)) > verticalTolerance then return false end

	local function canAirDismountToWater(pos)
		if not table.valid(pos) then return false end
		local canDismount, lx, ly, lz = CanAirDismountAtPos(pos.x, pos.y, pos.z)
		if not (canDismount and lx and ly and lz) then return false end
		local landing = { x = lx, y = ly, z = lz }
		if math.distance2d(ppos, landing) > scanDistance then return false end
		if math.distance2d(goal, landing) > scanDistance then return false end
		local isWater = self:ProbeWaterSurfaceAt(landing, verticalTolerance, "moveto-landing")
		if isWater then
			return true
		end
		isWater = self:ProbeWaterSurfaceAt(pos, verticalTolerance, "moveto-sample")
		if isWater and math.abs((landing.y or 0) - (pos.y or 0)) <= verticalTolerance then
			return true
		end
		return false
	end
	...
	local samples = {
		goal,
		ppos,
		{ x = (ppos.x + goal.x) * 0.5, y = (ppos.y + goal.y) * 0.5, z = (ppos.z + goal.z) * 0.5 },
	}
	for _, sample in ipairs(samples) do
		if canAirDismountToWater(sample) then
			if (gNavDebug and self.DebugLog) then
				self:DebugLog("water-surface-dismount-ready",
					"Water surface air dismount ready sample=" .. self:FormatGroundAcquirePoint(sample)
					.. " goal=" .. self:FormatGroundAcquirePoint(goal), 1000)
			end
			return true
		end
	end
...
function ml_navigation:IsLandingControllerWaterSurfaceReady(controller, ppos)
	if not (controller and controller.active and Player and Player.ismounted and IsFlying()) then return false end
	if not table.valid(controller.landing) then return false end
	ppos = ppos or (Player and Player.pos)
	if not table.valid(ppos) then return false end

	local verticalTolerance = math.max(tonumber(controller.request and controller.request.dismountHeightTolerance) or 3.5, 4)
	local dist2d = math.distance2d(ppos, controller.landing)
	local dist3d = math.distance3d(ppos, controller.landing)
	if dist2d > 3.5 or dist3d > 9.0 then return false end
	return self:ProbeWaterSurfaceAt(controller.landing, verticalTolerance, "landing-controller")
end
...
function ml_navigation:IsLandingOrActionHandoffActive()
	...
	if (ffnav.airborneGroundAcquire and ffnav.airborneGroundAcquire.active) then
		...
		elseif not self:IsAirborneGroundAcquireTaskActive(task, ppos) then
			self:ClearAirborneGroundAcquire()
			...
			if not (segment and segment.groundAcquire
				and segment.key == ffnav.airborneGroundAcquire.segmentKey
				and segment.index == ffnav.airborneGroundAcquire.pathindex) then
				self:ClearAirborneGroundAcquire()
				self:DisableAutoFollow(true, "AirborneGroundAcquireSegmentChanged")
			else
				return true
			end
		end
	end
	if (ffnav.airborneGroundWalkTaskKey) then
		local task = ml_task_hub and ml_task_hub:CurrentTask()
		local ppos = Player and Player.pos
		self:GetAirborneGroundWalkHandoffSegment(task, ppos)
	end
...
function ml_navigation:GetCurrentPathSegmentInfo(pathIndex, lastnode, nextnode, nextnextnode, task)
	...
	if table.valid(self.path) then
		lastnode = lastnode or self.path[pathIndex - 1] or self.startposition
		nextnode = nextnode or self.path[pathIndex]
		nextnextnode = nextnextnode or self.path[pathIndex + 1]
	end

	if table.valid(lastnode) and lastnode.type and lastnode.type2 and not lastnode.is_tagged then
		ml_navigation.TagNode(lastnode)
	end
	ml_navigation.TagNode(nextnode)
	ml_navigation.TagNode(nextnextnode)

	local function nodeKey(node)
		if not table.valid(node) then return "nil" end
		return tostring(math.round(node.x or 0, 2)) .. ":"
			.. tostring(math.round(node.y or 0, 2)) .. ":"
			.. tostring(math.round(node.z or 0, 2)) .. ":"
			.. tostring(node.type or 0) .. ":"
			.. tostring(node.flags or 0) .. ":"
			.. tostring(node.navconnectionid or 0)
	end
	...
	local segment = {
		...
		key = tostring(pathIndex) .. "|" .. nodeKey(nextnode) .. "|" .. nodeKey(nextnextnode),
		floor = false,
		water = false,
		dive = false,
		flight = false,
	...
	local fromFlight = table.valid(lastnode) and (lastnode.air == true or lastnode.air_avoid == true)
	local unstableTransition = self:IsUnstableFlightTransition(nextnode, nextnextnode)

	segment.water = (nextnode.is_cube == true and nextnode.water == true)
	segment.dive = segment.water
	segment.flight = (not segment.dive) and (
		(nextnode.floorcube == true and (nextnode.air == true or nextNextAir) and not unstableTransition)
		or (nextnode.is_cube == true and nextnode.air == true and not nextnode.floorcube)
	)
	segment.descent = (nextnode.floorcube == true and (nextnode.ground == true or nextNextGround) and (fromFlight or IsFlying()))
	segment.finalLanding = (task and self:IsInteractActionTask(task) and self:GetLandingRequest(task) ~= nil
...
function ml_navigation:FormatGroundAcquirePoint(point)
	if not table.valid(point) then return "nil" end
	return string.format("(%.1f,%.1f,%.1f)", tonumber(point.x) or 0, tonumber(point.y) or 0, tonumber(point.z) or 0)
end

function ml_navigation:FormatGroundAcquireState(segment, task, ppos, request)
	local navIndex = NavigationManager and NavigationManager.NavPathNode or nil
	local goal = task and self:GetAirborneGroundAcquireMoveToGoal(task) or nil
	local acquire = ffnav and ffnav.airborneGroundAcquire or nil
	local walkIndex = ffnav and ffnav.airborneGroundWalkIndex or nil
	local requestAnchor = request and request.anchor or nil
	local bits = {
		"seg=" .. tostring(segment and segment.index or "nil"),
		"path=" .. tostring(self.pathindex),
		"nav=" .. tostring(navIndex),
		"player=" .. self:FormatGroundAcquirePoint(ppos),
		"node=" .. self:FormatGroundAcquirePoint(segment and segment.node),
		"next=" .. self:FormatGroundAcquirePoint(segment and segment.nextnextnode),
		"goal=" .. self:FormatGroundAcquirePoint(goal),
		"anchor=" .. self:FormatGroundAcquirePoint(requestAnchor),
		"task=" .. tostring(task and task.name or "nil"),
		"flying=" .. tostring(IsFlying()),
		"mounted=" .. tostring(type(IsMounted) == "function" and IsMounted() or false),
		"request=" .. tostring(request and request.source or "nil"),
		"activeAcquire=" .. tostring(acquire and acquire.pathindex or "nil"),
		"walkAcquire=" .. tostring(walkIndex),
	}
	return table.concat(bits, " ")
end

function ml_navigation:IsSkippableAirborneGroundAcquireSegment(segment, task, ppos)
	if not (IsFlying() and segment and table.valid(segment.node) and table.valid(segment.nextnextnode)
		and table.valid(ppos)) then return false end

	local distToNode = math.distance2d(ppos, segment.node)
	if distToNode > 2.0 then return false end

	local goal = self:GetAirborneGroundAcquireMoveToGoal(task)
	if table.valid(goal) then
		local range = tonumber(task and task.range) or 1.5
		local gatherRange = tonumber(task and task.gatherRange) or 0
		local arrived2d = math.max(range + gatherRange, 2.0) + 0.75
		if math.distance2d(ppos, goal) <= arrived2d then return false end
	end

	if (gNavDebug and self.DebugLog) then
		self:DebugLog("airborne-ground-acquire-skip-start",
			"Skipping airborne ground acquire at current/start node "
			.. self:FormatGroundAcquireState(segment, task, ppos, nil), 1000)
	end
	return true
end

function ml_navigation:FindUpcomingGroundAcquireSegment(task, startIndex)
	if not table.valid(self.path) then return nil end
	startIndex = tonumber(startIndex or self.pathindex or (NavigationManager and NavigationManager.NavPathNode) or 1) or 1
	if startIndex < 1 then startIndex = 1 end
	local ppos = Player and Player.pos

	for index, node in pairsByKeys(self.path) do
		local i = tonumber(index)
		if i and i >= startIndex and table.valid(node) then
			local lastnode = self.path[i - 1] or self.startposition
			local nextnode = self.path[i]
			local nextnextnode = self.path[i + 1]
			local segment = self:GetCurrentPathSegmentInfo(i, lastnode, nextnode, nextnextnode, task)
			if segment and segment.groundAcquire then
				if not self:IsSkippableAirborneGroundAcquireSegment(segment, task, ppos) then
					return segment
				end
			end
			if segment and segment.finalLanding then
				return nil
			end
		end
	end
...
function ml_navigation:ShouldDeferTaskLandingToGroundAcquire(task, ppos, request)
	if not (IsFlying() and task and self:IsInteractActionTask(task)) then return false end
	if request and request.source == "action" then return false end
	local startIndex = tonumber(self.pathindex or (NavigationManager and NavigationManager.NavPathNode) or 1) or 1
	local segment = self:FindUpcomingGroundAcquireSegment(task, startIndex)
	if not segment then return false end
	if gNavDebug and self.DebugLog then
		self:DebugLog("task-landing-defer-ground",
			"Task landing deferred to ground path segment "
			.. self:FormatGroundAcquireState(segment, task, ppos, request)
			.. " searchStart=" .. tostring(startIndex), 1000)
	end
	return true
end
...
function ml_navigation:ClearAirborneGroundWalkSegment()
	ffnav.airborneGroundWalkSegmentKey = nil
	ffnav.airborneGroundWalkIndex = nil
	ffnav.airborneGroundWalkTaskKey = nil
	ffnav.airborneGroundWalkReason = nil
end

function ml_navigation:IsAirborneGroundWalkContinuationSegment(segment)
	return segment and (segment.groundAcquire or segment.finalLanding)
end

function ml_navigation:GetPathLastIndex()
	if not table.valid(self.path) then return nil end
	local lastIndex = nil
	for index, node in pairsByKeys(self.path) do
		local i = tonumber(index)
		if i and table.valid(node) and (not lastIndex or i > lastIndex) then
			lastIndex = i
		end
	end
	return lastIndex
...
function ml_navigation:GetCurrentWalkingPathSegmentInfo(task, ppos)
	if IsFlying() or (type(IsDiving) == "function" and IsDiving()) then return false end
	if not table.valid(self.path) then return false end

	local segment = self:GetCurrentPathSegmentInfo(nil, nil, nil, nil, task)
	if segment and table.valid(segment.node) then
		if segment.flight or segment.dive or segment.descent then return false end
		if segment.floor or segment.groundAcquire or segment.finalLanding then return segment end

		local node = segment.node
		if node.is_floor == true and node.is_cube ~= true then return segment end
		return false
	end

	local pathIndex = tonumber(self.pathindex or (NavigationManager and NavigationManager.NavPathNode) or 1) or 1
	local lastIndex = self:GetPathLastIndex()
	if not (lastIndex and pathIndex > lastIndex) then return false end

	local lastSegment = self:GetCurrentPathSegmentInfo(lastIndex,
		self.path[lastIndex - 1] or self.startposition, self.path[lastIndex], self.path[lastIndex + 1], task)
	if not (lastSegment and table.valid(lastSegment.node)) then return false end
	if lastSegment.flight or lastSegment.dive or lastSegment.descent then return false end
	if lastSegment.floor or lastSegment.groundAcquire or lastSegment.finalLanding then return lastSegment end
	if lastSegment.node.is_floor == true and lastSegment.node.is_cube ~= true then return lastSegment end
	return false
end

function ml_navigation:IsCurrentWalkingPathSegment(task, ppos)
	return self:GetCurrentWalkingPathSegmentInfo(task, ppos) ~= false
end

function ml_navigation:GetAirborneGroundAcquireTaskKey(task)
	if not task then return nil end
	local goal = task.gatePos or task.pos
	if not table.valid(goal) then goal = {} end
	return tostring(task.started or 0) .. ':' .. tostring(task.name or '') .. ':'
		.. tostring(math.round(goal.x or 0, 1)) .. ':' .. tostring(math.round(goal.y or 0, 1)) .. ':' .. tostring(math.round(goal.z or 0, 1))
end

function ml_navigation:IsAirborneGroundAcquireStateForTask(state, task)
	if not (state and state.taskKey) then return true end
	return state.taskKey == self:GetAirborneGroundAcquireTaskKey(task)
end
...
function ml_navigation:MarkAirborneGroundAcquireLanded(state)
	...
	if (gNavDebug and self.DebugLog) then
		...
		self:DebugLog("airborne-ground-walk-segment",
			...
			.. self:FormatGroundAcquireState(segment, task, ppos, nil)
			.. " landedSeg=" .. tostring(state.pathindex)
			.. " landedTarget=" .. self:FormatGroundAcquirePoint(state.target), 1000)
	end
	return true
end

function ml_navigation:GetAirborneGroundWalkHandoffSegment(task, ppos)
	if not ffnav.airborneGroundWalkTaskKey then return nil end
	task = task or (ml_task_hub and ml_task_hub:CurrentTask())
	local taskKey = self:GetAirborneGroundAcquireTaskKey(task)
	if taskKey ~= ffnav.airborneGroundWalkTaskKey then
		self:ClearAirborneGroundWalkSegment()
		return nil
	end
	...
	local segment = self:GetCurrentPathSegmentInfo(nil, nil, nil, nil, task)
	if not (self:IsAirborneGroundWalkContinuationSegment(segment) and segment.key == ffnav.airborneGroundWalkSegmentKey
		and segment.index == ffnav.airborneGroundWalkIndex) then
		if self:IsAirborneGroundWalkContinuationSegment(segment) then
			ffnav.airborneGroundWalkIndex = segment.index
			ffnav.airborneGroundWalkSegmentKey = segment.key
			ffnav.airborneGroundWalkTaskKey = taskKey
			ffnav.airborneGroundWalkReason = ffnav.airborneGroundWalkReason or "GroundPath"
			if (gNavDebug and self.DebugLog) then
				self:DebugLog("airborne-ground-walk-realign",
					"Realigned acquired ground walk "
					.. self:FormatGroundAcquireState(segment, task, ppos, nil), 1000)
			end
		else
			self:ClearAirborneGroundWalkSegment()
...
function ml_navigation:IsAirborneGroundWalkHandoffActive(task, ppos)
	return self:GetAirborneGroundWalkHandoffSegment(task, ppos) ~= nil
end

function ml_navigation:AdvanceAirborneGroundWalkSegment(task, pathIndex)
	if not ffnav.airborneGroundWalkTaskKey then return false end
	task = task or (ml_task_hub and ml_task_hub:CurrentTask())
	local taskKey = self:GetAirborneGroundAcquireTaskKey(task)
	if taskKey ~= ffnav.airborneGroundWalkTaskKey then
		self:ClearAirborneGroundWalkSegment()
	...
	pathIndex = tonumber(pathIndex or self.pathindex or (NavigationManager and NavigationManager.NavPathNode) or 1) or 1
	local segment = self:GetCurrentPathSegmentInfo(pathIndex, nil, nil, nil, task)
	if self:IsAirborneGroundWalkContinuationSegment(segment) then
		ffnav.airborneGroundWalkIndex = segment.index
		ffnav.airborneGroundWalkSegmentKey = segment.key
		ffnav.airborneGroundWalkTaskKey = taskKey
		ffnav.airborneGroundWalkReason = ffnav.airborneGroundWalkReason or "GroundPath"
		if (gNavDebug and self.DebugLog) then
			self:DebugLog("airborne-ground-walk-advance",
				"Continuing acquired ground walk "
				.. self:FormatGroundAcquireState(segment, task, ppos, nil), 1000)
		end
		return true
	end
...
function ml_navigation:ShouldWalkAirborneGroundAcquiredSegment(node)
	if not table.valid(node) then return false end
	return self:IsAirborneGroundWalkHandoffActive()
end
function ml_navigation:GetAirborneGroundAcquireMoveToGoal(task)
	if not task then return nil end
	return task.gatePos or task.pos
end
...
function ml_navigation:IsAirborneGroundAcquireTaskActive(task, ppos)
	...
	if table.valid(ppos) then
		local range = tonumber(task.range) or 1.5
		local gatherRange = tonumber(task.gatherRange) or 0
		local required2d = math.max(range + gatherRange, 2.0)
		local threshold2d = self.GetMovementThresholds()
...
function ml_navigation:GetAirborneGroundAcquireTarget(ppos, nextnode)
	...
	local function normalize(dx, dz)
		local len = math.sqrt((dx * dx) + (dz * dz))
		if len <= 0.1 then return nil, nil end
	...
	local dx, dz = nil, nil
	if table.valid(toward) then
		dx, dz = normalize((toward.x or ppos.x) - (ppos.x or 0), (toward.z or ppos.z) - (ppos.z or 0))
	end
	if (not dx) and ppos.h then
		dx, dz = normalize(math.cos(ppos.h), math.sin(ppos.h))
	end
	dx = dx or 1
	dz = dz or 0
	...
	if table.valid(meshPoint) then
		...
		if (not meshDistance or meshDistance <= 80) and math.distance2d(ppos, meshPoint) <= 8 then
			return { x = meshPoint.x, y = self:GetLandingSurfaceDispatchY(meshPoint.y), z = meshPoint.z, landing = true, surfaceY = meshPoint.y }
		end
	end

	local meshPos = FindClosestMesh(ppos, 30, false)
	if table.valid(meshPos) and math.distance2d(ppos, meshPos) <= 8 then
		return { x = meshPos.x, y = self:GetLandingSurfaceDispatchY(meshPos.y), z = meshPos.z, landing = true, surfaceY = meshPos.y }
	end
	return nil
end
...
function ml_navigation:IsAirborneGroundAcquireReady(ppos, segment, quiet)
	...
	if (gNavDebug and self.DebugLog and not quiet) then
...
function ml_navigation:StartAirborneGroundAcquire(reason, nextnode)
	...
	if (type(IsDiving) == 'function' and IsDiving()) then return false end
	if self:IsRealLandingOrActionHandoffActive() then return false end
	local task = ml_task_hub and ml_task_hub:CurrentTask()
	local ppos = Player and Player.pos
	if not self:IsAirborneGroundAcquireTaskActive(task, ppos) then return false end
	...
	if self:IsSkippableAirborneGroundAcquireSegment(segment, task, ppos) then return false end
	...
	ffnav.airborneGroundAcquire = {
		active = true,
		target = target,
		pathindex = segment.index,
	...
	ffnav.isdescending = false
	self:ResetLandingController()
	self:ResetFlightActionThrottle(false)
	if (ffxiv_unstuck and ffxiv_unstuck.Reset) then
		ffxiv_unstuck.Reset()
	end
	if (gNavDebug and self.DebugLog) then
		self:DebugLog("airborne-ground-acquire-start",
			"Airborne ground acquire start "
			.. self:FormatGroundAcquireState(segment, task, ppos, nil)
			.. " target=" .. self:FormatGroundAcquirePoint(target)
			.. " reason=" .. tostring(reason), 1000)
	end
	if (not ffnav.airborneGroundAcquireLog or TimeSince(ffnav.airborneGroundAcquireLog) > 1000) then
		d('[Navigation]: Airborne MoveTo landing to acquire ground target=(' .. string.format('%.1f, %.1f, %.1f', target.x or 0, target.y or 0, target.z or 0) .. ').')
		ffnav.airborneGroundAcquireLog = Now()
	end
	return true
end
function ml_navigation:DriveAirborneGroundAcquire(ppos)
	local state = ffnav.airborneGroundAcquire
	if not (state and state.active) then return false end
	local task = ml_task_hub and ml_task_hub:CurrentTask()
	ppos = ppos or (Player and Player.pos)
	if not self:IsAirborneGroundAcquireStateForTask(state, task) then
		self:ClearAirborneGroundAcquire()
		self:ClearAirborneGroundWalkSegment()
		self:DisableAutoFollow(true, "AirborneGroundAcquireTaskChanged")
		return false
	...
	if self:IsRealLandingOrActionHandoffActive() then
		self:ClearAirborneGroundAcquire()
		self:DisableAutoFollow(true, "AirborneGroundAcquireHandoff")
		return false
	end

	local segment = self:GetCurrentPathSegmentInfo(nil, nil, nil, nil, task)
	if not (segment and segment.groundAcquire and segment.key == state.segmentKey and segment.index == state.pathindex) then
		self:ClearAirborneGroundAcquire()
		self:DisableAutoFollow(true, "AirborneGroundAcquireSegmentChanged")
		return false
	end
	...
	if not table.valid(state.target) then
		self:ClearAirborneGroundAcquire()
		self:DisableAutoFollow(true, "AirborneGroundAcquireInvalid")
		return false
	end
		...
		if shouldRetarget then
			...
			if table.valid(target) then
				local changed = (math.distance2d(target, state.target) > 1.0)
					or (math.abs((target.y or 0) - (state.target.y or 0)) > 1.0)
				state.target = target
				if changed then
					self:ResetAutoFollowState()
					if (not ffnav.airborneGroundAcquireRetargetLog or TimeSince(ffnav.airborneGroundAcquireRetargetLog) > 1000) then
						d('[Navigation]: Airborne MoveTo landing retarget=(' .. string.format('%.1f, %.1f, %.1f', target.x or 0, target.y or 0, target.z or 0) .. ').')
						ffnav.airborneGroundAcquireRetargetLog = Now()
					end
				end
			end
		end
	...
	self.GUI = self.GUI or {}
	self.GUI.lastAction = "Landing to acquire ground"
	self:SuppressUnstuck(1000, "AirborneGroundAcquire")
	self:DispatchAutoFollowNode(state.target, true)
	return true
end
function ml_navigation:GetLandingHandoffDebugState()
	local now = Now()
	local function remaining(value)
		if (value and value > now) then return tostring(value - now) end
		return "0"
	end
	local controller = ffnav.landingController
	local controllerState = "nil"
	if (controller) then
		controllerState = tostring(controller.phase) .. ":" .. tostring(controller.active)
	end
...
function ml_navigation:ShouldTaskLandingOwnAirborne(task, ppos)
	if not (IsFlying() and self:IsInteractActionTask(task) and ppos) then return false end
	if self:IsLandingDismountOrActionHandoffActive() then return false end
	local request = self:GetLandingRequest(task)
	if not request then return false end

	local controller = ffnav.landingController
	if controller and controller.active and controller.phase and controller.phase ~= 'idle' then
		if self:ShouldDeferTaskLandingToGroundAcquire(task, ppos, controller.request) then
			self:ResetLandingController()
			return false
		end
		if controller.phase == 'blocked' then
	...
	if self:ShouldDeferTaskLandingToGroundAcquire(task, ppos, request) then
		return false
	end
...
function ml_navigation:NeedsLandingApproachFlight(task, request, ppos)
	...
	if self:IsCurrentWalkingPathSegment(task, ppos) then
		if (gNavDebug and self.DebugLog) then
			local segment = self:GetCurrentWalkingPathSegmentInfo(task, ppos)
			self:DebugLog("landing-flight-suppressed-walking-segment",
				"Landing approach flight suppressed by current walking segment "
				.. self:FormatGroundAcquireState(segment, task, ppos, request), 1000)
		end
		return false
	end
	if self:IsLandingDismountOrActionHandoffActive() then return false end
	if self:IsAirborneGroundWalkHandoffActive(task, ppos) then
		if (gNavDebug and self.DebugLog) then
			local segment = self:GetAirborneGroundWalkHandoffSegment(task, ppos)
			self:DebugLog("landing-flight-suppressed-ground-walk",
				"Landing approach flight suppressed while walking acquired ground segment "
				.. self:FormatGroundAcquireState(segment, task, ppos, request), 1000)
		end
		return false
	end
...
function ml_navigation:NeedsMountedLandingTakeoff(task, request, ppos)
	...
	local segment = self:GetCurrentPathSegmentInfo(nil, nil, nil, nil, task)
	if segment and segment.flight then
		if gNavDebug and self.DebugLog then
			self:DebugLog("landing-takeoff-defer-flight-path",
				"Landing takeoff deferred to flight path segment index=" .. tostring(segment.index)
				.. " task=" .. tostring(task and task.name or "nil"), 1000)
		end
		return false
	end
...
function ml_navigation:PrepareLandingTakeoff(task, ppos)
	...
	if self:IsCurrentWalkingPathSegment(task, ppos) then
		...
		if (gNavDebug and self.DebugLog) then
			local segment = self:GetCurrentWalkingPathSegmentInfo(task, ppos)
			self:DebugLog("landing-takeoff-suppressed-walking-segment",
				"Landing takeoff suppressed by current walking segment "
				.. self:FormatGroundAcquireState(segment, task, ppos, request), 1000)
		end
		return false
	end
...
function ml_navigation:LandForAction(anchor, maxDistance3d, source)
	...
	self:SuppressUnstuck(2500, tostring(source or 'action'))
...
function ml_navigation:IsInteractActionTask(task)
	...
	if (task.interact and task.interact ~= 0) then return true end
	if (task.contentid and task.contentid ~= 0 and task.interactRange ~= nil) then return true end
	return false
end
...
function ml_navigation:BeginInteractActionHandoff(source, commitInteract)
	...
	self.canPath = false
	if (not ffnav.interactActionHandoffLog or TimeSince(ffnav.interactActionHandoffLog) > 1000) then
		d("[Navigation] Interact action handoff: pathing disabled while interacting.")
		ffnav.interactActionHandoffLog = Now()
	end
end
...
function ml_navigation:IsGoalClose(ppos,node,lastnode)
	...
	if (node.navconnectionid and node.navconnectionid ~= 0) then
		if (NavigationManager.ShowCells == nil ) then
			...
			if (nc and nc.details and nc.details.subtype) then
...
function ml_navigation:ResetOMCHandler()
	...
	self.omc_jumptimer = nil
	self.omc_prevx = nil
	self.omc_prevz = nil
	self.omc_prevtick = nil
	self.omc_atedge = false
...
function ml_navigation_exact.Reset()
	...
	ml_navigation_exact.omc_jumptimer = nil
	ml_navigation_exact.omc_prevx = nil
	ml_navigation_exact.omc_prevz = nil
	ml_navigation_exact.omc_prevtick = nil
	ml_navigation_exact.omc_atedge = false
...
function ml_navigation_exact.ResetOMCState()
	...
	ml_navigation_exact.omc_traveldist = 0
	ml_navigation_exact.omc_startheight = nil
	ml_navigation_exact.omc_centeredsince = nil
end
...
function Player:Stop(resetpath)
	...
	-- task abandonment) also tears down exact movement instead of leaving it
	-- driving the player toward the old goal. StopExact no-ops when inactive.
	Player:StopExact()
	...
	if ((ticks - (ml_navigation.lastupdate or 0)) > 50) then
		...
		if (not (ml_navigation.CanRun() and ml_navigation.canPath and not ml_navigation.debug)) then
			...
			if ml_navigation.CanRun() and not ml_navigation.debug then
				guardTask = ml_task_hub and ml_task_hub:CurrentTask()
				guardPos = Player and Player.pos
				if ml_navigation:DriveAirborneGroundAcquire(guardPos) then return end
			end
			...
			if ml_navigation.CanRun() and not ml_navigation.debug then
				local landing = ffnav and ffnav.landingController
				if guardTask and IsFlying() and landing and landing.active
					and ml_navigation:ShouldDeferTaskLandingToGroundAcquire(guardTask, guardPos, landing.request) then
					ml_navigation:ResetLandingController()
					landing = ffnav and ffnav.landingController
				end
				if landing and landing.active and landing.phase and landing.phase ~= 'idle'
					and ml_navigation:DriveFlyingLanding(landing, landing.request or {}, guardPos) then return end
				if guardTask and (guardTask.name == "MOVETOINTERACT" or (guardTask.interact and guardTask.interact ~= 0)) then
					if ml_navigation.TryInteractAutoFollow(guardTask) then
						return
					end
					if not IsFlying() and ml_navigation.NeedsInteractCloseApproach(guardTask) then
						return
					end
				end
			end
			if not ml_navigation.canPath then
				local landing = ffnav and ffnav.landingController
				if landing and landing.active then
					ml_navigation:ResetLandingController()
				end
			end
		...
		if ( ml_navigation.CanRun() and ml_navigation.canPath and not ml_navigation.debug) then
			...
			local ppos = Player.pos
			local currentTask = ml_task_hub and ml_task_hub:CurrentTask()
			if ml_navigation:DriveAirborneGroundAcquire(ppos) then return end
			local landingController = ffnav and ffnav.landingController
			if IsFlying() and landingController and landingController.active
				and ml_navigation:ShouldDeferTaskLandingToGroundAcquire(currentTask, ppos, landingController.request) then
				ml_navigation:ResetLandingController()
				landingController = ffnav and ffnav.landingController
			end
			if landingController and landingController.active and landingController.phase and landingController.phase ~= 'idle'
				and ml_navigation:DriveFlyingLanding(landingController, landingController.request or {}, ppos) then return end
			local taskLandingOwnsAirborne = IsFlying() and ml_navigation:ShouldTaskLandingOwnAirborne(currentTask, ppos)
			if taskLandingOwnsAirborne and ml_navigation:UpdateFlyingLanding(currentTask, ppos) then return end
			if (not IsFlying()) and ml_navigation:PrepareLandingTakeoff(currentTask, ppos) then return end

			ml_navigation.GUI.pathHops = 0
			ml_navigation.GUI.currentIndex = 0
			ml_navigation.GUI.nextNodeDistance = 0
			ml_navigation.GUI.lastAction = ""
			...
			if (not ffnav.IsProcessing()) then
				...
				if ( table.valid(ml_navigation.path) and ml_navigation.path[ml_navigation.pathindex] ~= nil) then
					...
					local currentSegment = ml_navigation:GetCurrentPathSegmentInfo(ml_navigation.pathindex, lastnode, nextnode, nextnextnode, currentTask)
					if IsFlying() and not taskLandingOwnsAirborne and currentSegment and currentSegment.groundAcquire
						and ml_navigation:StartAirborneGroundAcquire("GroundPath", nextnode)
						and ml_navigation:DriveAirborneGroundAcquire(ppos) then return end
					...
					if ( self.omc_id ) then
						...
						if (not MIsLocked() and not isInteractOMC) then
							...
							-- For a jump OMC still approaching the start node (not yet at
							-- the edge), measure progress toward the START. Otherwise a short
							-- landing on the previous step re-arms this OMC ~1.0 out and the
							-- recovery walk back onto the start node (which does not shrink the
						...
						-- OMC Jump (subtype 1)
						-- History of jump handling added here:
						--  - Two-phase jump: Phase A drives onto/through the start node and launches;
						--    Phase B aims the landing point until grounded, then advances pathindex.
						--  - needCarry: horizontal gaps (gap2d > stepH) must launch with momentum;
						--    vertical hops may launch from a standstill.
						--  - omc_atedge latch + runThrough: once at the start, commit to running through
						--    toward to_pos so the player cannot ping-pong/stall.
						--  - centered gate: vertical hops launch only from ON the node (<=0.25) to keep
						--    full vertical reach for tall steps.
						--  - Momentum-preferring launch: vertical hops wait (while centered) for real
						--    movement, then launch; fall back after 200ms if no run-up is possible.
						--  - REJUMP net: if a jump falls short, re-arm Phase A.
						if ( ncsubtype == 1 ) then
							ml_navigation.GUI.lastAction = "Jump NavConnection"
							local landing2d = ml_navigation.NavPointReachedDistances["2dwalk"]
							local landHeightTolerance = 0.5
							local reach = math.max(ncradius or 0.5, 0.5)
							...
							if ( not ml_navigation.omc_startheight ) then
								...
								local speed = 0
								if ( ml_navigation.omc_prevtick ) then
									local dt = ticks - ml_navigation.omc_prevtick
									if ( dt >= 1 ) then
										speed = math.distance2d(ppos, { x = ml_navigation.omc_prevx, y = ppos.y, z = ml_navigation.omc_prevz }) / dt
									end
								end
								...
								if ( runThrough ) then
									...
									if ( centered and notDescending and not Player:IsJumping() and launchOk ) then
										ml_navigation.omc_centeredsince = nil
								...
								-- Phase B: airborne — aim the landing point until reached.
						...
						if (nextnode.type == GLOBAL.NODETYPE.CUBE or isCubeCon) then
							...
							elseif (not IsFlying() and CanFlyInZone()) then
								...
								if currentWalkingSegment then
									...
									if (gNavDebug and ml_navigation.DebugLog) then
								...
								if ml_navigation:ShouldWalkAirborneGroundAcquiredSegment(nextnode) then
									...
									if (ml_navigation:IsGoalClose(ppos,nextnode,lastnode)) then
										...
										NavigationManager.NavPathNode = ml_navigation.pathindex
										ml_navigation:ResetAutoFollowState()
										ml_navigation:AdvanceAirborneGroundWalkSegment(nil, ml_navigation.pathindex)
									else
										ml_navigation:DispatchAutoFollowNode(nextnode, true)
									end
									return
								end
								if ml_navigation:IsLandingOrActionHandoffActive() then
									ml_navigation:DisableAutoFollow(true, "LandingActionHandoffFlight")
									return
								end
								if (not Player.ismounted) then
									if (Player:IsMoving()) then
									...
									if (Player:IsMoving()) then
										...
										local isFlightCon = (currentSegment and currentSegment.flight) or false
										local shouldAscend = isFlightCon or (nextnode.is_cube and not nextnode.floorcube and not nextnode.water)
							...
							local walkingAcquiredGround = ml_navigation:ShouldWalkAirborneGroundAcquiredSegment(nextnode)
							ml_navigation.GUI.lastAction = walkingAcquiredGround and "Walk acquired ground segment" or "Walk to Node"
							if (ml_navigation:IsGoalClose(ppos,nextnode,lastnode)) then
								ml_navigation.lastconnectionid = nextnode.navconnectionid
								ml_navigation.lastconnectiontimer = Now()
								ml_navigation.pathindex = ml_navigation.pathindex + 1
...
function ml_navigation_exact.Navigate(event, ticks)
	...
	-- Engage nav-connection (OMC) nodes from the same wide range MoveTo uses.
	-- A short OMC landing can leave the player up to ~landing2d (1.0) from the
	-- next OMC's start; the tight 0.2 node threshold would never trigger there, so
	-- the player runs into the step forever with no Phase A and no recovery. Only
	-- connection nodes are widened, so normal waypoints keep precise 0.2 arrival.
	local reachThreshold = self.threshold
	if (nextnode.navconnectionid and nextnode.navconnectionid ~= 0) then
		reachThreshold = math.max(self.threshold, ml_navigation.NavPointReachedDistances["2dwalk"] or 1.0)
	end

	if (dist2d <= reachThreshold and withinVertical) then
...
function ml_navigation_exact.HandleOMC(ppos, ticks)
	...
	local from_pos, to_pos, ncradius, ncsubtype
	...
	if (not MIsLocked()) then
		...
		-- For a jump OMC still approaching the start node (not yet at the edge),
		-- measure progress toward the START so a recovery walk after a short
	...
	-- OMC Jump (subtype 1)
	-- Same jump handling as the MoveTo path (see ml_navigation OMC Jump history):
	-- two-phase launch, needCarry, omc_atedge/runThrough latch, centered gate,
	-- momentum-preferring launch, and REJUMP net.
	if (ncsubtype == 1) then
		local landing2d = ml_navigation.NavPointReachedDistances["2dwalk"]
		local landHeightTolerance = 0.5
		local reach = math.max(ncradius or 0.5, 0.5)
		...
		if (not self.omc_startheight) then
			...
			local speed = 0
			if (self.omc_prevtick) then
				local dt = ticks - self.omc_prevtick
				if (dt >= 1) then
					speed = math.distance2d(ppos, { x = self.omc_prevx, y = ppos.y, z = self.omc_prevz }) / dt
				end
			end
