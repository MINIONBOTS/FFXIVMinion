...
-- THIS FILE HOLDS RIGHT NOW 2 VERSIONS OF THE SAME CODE, UPPER ONE IS FOR 64bit the LOWER ONE for 32bit backwards compabanana until 32bit shit is gone.

if ( NavigationManager.CubeRenderDistance ~= nil ) then
...
function Player:MoveTo(x, y, z, navpointreacheddistance, randompath, smoothturns, navigationmode, cubesoff)
	...
	--[[
	This is already done in ml_navigation, adjust ml_navigation.NewPathDistanceThresholds if you want different "new goal dist" thresholds
	...
	ffxiv_navigation line 691 -> whe you have a partial path and the actual goal is still 1000 units away or when the point to goto is "in the air" (gather node hanging high) , it just adds the final goal as endpoint  (which btw the c++ part is already doing when it is within an acceptable dist threshold!) ,
	resulting in the bot walking plain straight without any kind of path into a wall or doing a nice spinny dance on the spot , trying to reach that unreachable goal-
...
function ffnav.Ascend()	
	...
	ffnav.process = {
		...
		followall = function () end  --d("WHAT IS THIS ?!?!?  Stopping the player half way upwards completely breaks the navigation logic!?") Player:Stop() end
	}
end
-- END OF 64 BIT CODE
...
-- 32 BIT CODE BELOOOOOOOOOOOOOOOOOOOOOOW
else
-- Extends minionlib's ml_navigation.lua by adding the game specific navigation handler
